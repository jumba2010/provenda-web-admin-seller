...
const MarketReview = (props) => {
  ...
  const columns = [
...

    {
      ...
      dataIndex: 'rate',
      valueType: 'text',
      render:(text)=><Rate disabled value={parseFloat(text)} allowHalf />,
    },
  ...
  useEffect(() => {
    dispatch({
      type: 'review/fetchMarketReviews',
    ...
    <PageHeaderWrapper>
     ...
     <Card extra={<span>
      <>
          ...
          <ExportXLS dataset={getXLSData(reviews)} sheetName={formatMessage({ id: 'menu.market.reviews'})} collumns={getXLSColumns(columns,true)} />
...
export default connect(({ review, loading,user }) => ({
  reviews: review.marketReviews,
  currentUser:user.currentUser,
  fetching: loading.effects['reviews/fetchMarketReviews'],
}))(MarketReview);
